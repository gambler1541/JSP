...
%>
	<tr align="center" valign="middle">
		...
		<td colspan="2" align="right"><font size="3"> 글 개수: ${listcount }</font></td>
	</tr>
...
<!-- 검색 기능 추가 -->
	<tr>
		<td colspan="4" align="center">
			<br />
			<form action="BoardList.bo" method="post" name="srchForm">
				<select name="srchFlds" id="">
					...
					<option value="au" <%= srchFlds.equals("au") ? "selected='selected'" : "" %>>작성자</option>
					<option value="con" <%= srchFlds.equals("con") ? "seleceted='selected'" : "" %>>내용</option>
				</select>
				<input type="text" name="srchKey" size="20" maxlength="50" value="<%= srchKey %>" />
				<input type="button" value="검색" onClick="submitSrchForm()" />
				<input type="text"  value="리셋" onClick="resetSrchForm()"/>
			</form>
			<br />
		</td>
		<td />
...
%>
	<tr align="center" height="20">
		...
		<% for (int a = startpage; a<=endpage; a++) {
			if(a==nowpage) {%>
				[<%= a %>]&nbsp;
		<% }else{ %>
			<a href="./BoardListbo?srchFlds=<%= srchFlds %>&srchKey=<%= srchKey %>&page=<%= a %>">[<%= a %>]</a> &nbsp;
		<% } %>
	<% } %>
	...
	<% if (nowpage >= maxpage){ %>
		[다음]
	<% } else { %>
		<a href="./BoardListbo?srchFlds=<%= srchFlds %>&srchKey=<%= srchKey %>&page=<%= nowpage + 1 %>">[다음]</a>
	<% } %>
		</td>
	</tr>
	<% } else { %>
